...
const ApiService = {
    ...
    getPlatById: (restaurantId, dishId) =>
        AxiosInstance.get(`/restaurant/${restaurantId}/dishes/${dishId}`).then((res) => res.data), // R�cup�rer un plat sp�cifique d'un restaurant

    createPlat: (restaurantId, data) =>
        AxiosInstance.post(`/restaurant/${restaurantId}/dishes`, data).then((res) => res.data), // Cr�er un plat pour un restaurant

    updatePlat: (restaurantId, dishId, data) =>
        AxiosInstance.put(`/restaurant/${restaurantId}/dishes/${dishId}`, data).then((res) => res.data), // Mettre � jour un plat sp�cifique d'un restaurant

    deletePlat: (restaurantId, dishId) =>
        AxiosInstance.delete(`/restaurant/${restaurantId}/dishes/${dishId}`).then((res) => res.data), // Supprimer un plat sp�cifique d'un restaurant
